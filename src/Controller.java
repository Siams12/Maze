...
public class Controller {
	...
	public void go() {
	    ...
	    cords = mazeController.getCordsOut();
	    for (int i = 0; i < mazeController.getHeight() ; i++) {
			for (int j = 0; j < mazeController.getWidth(); j++) {
				if (mazeController.get(i,j) != '#') {
					mazeController.set(' ', i, j);
				}
			}
	    }
}}}
