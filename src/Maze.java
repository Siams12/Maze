...
//Controller algorithm
	...
	private void movement(int clearSpots, int row, int col) {
		if(clearSpots == 1) {
	...
	public boolean didChangeDirection() {
		int oneRow = recentMoves.get(0).getRow();
		int oneCol = recentMoves.get(0).getColumn();
		int twoRow = recentMoves.get(1).getRow();
		int twoCol = recentMoves.get(1).getColumn();
		int threeRow = recentMoves.get(2).getRow();
		int threeCol = recentMoves.get(2).getColumn();
		if(oneRow == twoRow & twoRow == threeRow) {
			return false;
		}
		if(oneCol == twoCol & twoCol == threeCol) {
			return false;
		}
	...
	public String solve (int startrow, int startcol, int Endrow, int Endcol) {
		...
		if (countClearSpots(startrow,startcol) == 0 && findPosition('$') == null) {
			return null;
		}
